...
import { Plus, Edit2, Trash2, Search, HelpCircle, CheckCircle2 } from "lucide-react";
...
export default function QuestionManagement() {
  const [questions, setQuestions] = useLocalStorage<Question[]>("questions", MOCK_QUESTIONS);
  ...
  const handleSave = () => {
    if (editingQuestion) {
      ...
      setQuestions([...questions, { ...formData, id: Math.random().toString(36).substr(2, 9) }]);
    }
    setIsOpen(false);
  };
            ...
            <CardHeader>
              <div className="flex justify-between items-start">
                <span className="text-xs font-bold uppercase tracking-widest text-primary bg-primary/10 px-2 py-1 rounded">
                  {classes.find(c => c.id === q.classId)?.name}
                </span>
